// Code generated by mockery v2.53.3. DO NOT EDIT.

package mocks

import (
	context "context"

	entity "github.com/paincake00/microservices-go/order/internal/entity"

	mock "github.com/stretchr/testify/mock"
)

// InventoryService is an autogenerated mock type for the InventoryService type
type InventoryService struct {
	mock.Mock
}

type InventoryService_Expecter struct {
	mock *mock.Mock
}

func (_m *InventoryService) EXPECT() *InventoryService_Expecter {
	return &InventoryService_Expecter{mock: &_m.Mock}
}

// ListParts provides a mock function with given fields: ctx, partUuids
func (_m *InventoryService) ListParts(ctx context.Context, partUuids []string) ([]*entity.Part, error) {
	ret := _m.Called(ctx, partUuids)

	if len(ret) == 0 {
		panic("no return value specified for ListParts")
	}

	var r0 []*entity.Part
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, []string) ([]*entity.Part, error)); ok {
		return rf(ctx, partUuids)
	}
	if rf, ok := ret.Get(0).(func(context.Context, []string) []*entity.Part); ok {
		r0 = rf(ctx, partUuids)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*entity.Part)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context, []string) error); ok {
		r1 = rf(ctx, partUuids)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// InventoryService_ListParts_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'ListParts'
type InventoryService_ListParts_Call struct {
	*mock.Call
}

// ListParts is a helper method to define mock.On call
//   - ctx context.Context
//   - partUuids []string
func (_e *InventoryService_Expecter) ListParts(ctx interface{}, partUuids interface{}) *InventoryService_ListParts_Call {
	return &InventoryService_ListParts_Call{Call: _e.mock.On("ListParts", ctx, partUuids)}
}

func (_c *InventoryService_ListParts_Call) Run(run func(ctx context.Context, partUuids []string)) *InventoryService_ListParts_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].([]string))
	})
	return _c
}

func (_c *InventoryService_ListParts_Call) Return(_a0 []*entity.Part, _a1 error) *InventoryService_ListParts_Call {
	_c.Call.Return(_a0, _a1)
	return _c
}

func (_c *InventoryService_ListParts_Call) RunAndReturn(run func(context.Context, []string) ([]*entity.Part, error)) *InventoryService_ListParts_Call {
	_c.Call.Return(run)
	return _c
}

// NewInventoryService creates a new instance of InventoryService. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewInventoryService(t interface {
	mock.TestingT
	Cleanup(func())
}) *InventoryService {
	mock := &InventoryService{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
