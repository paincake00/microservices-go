...
// Category описывает категорию предмета
type Category int32
...
// GetPartRequest входные данные для GetPart
...
// GetPartResponse выходные данные для GetPart
...
// ListPartsRequest входные данные для ListParts
...
// ListPartsResponse выходные данные для ListParts
...
// PartsFilter описывает фильтр для выбора нужных Part
...
// Part описывает модель предмета со склада
type Part struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Uuid          string                 `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
...
// Dimensions описывает размеры предмета
...
// Manufacturer описывает производителя предмета
...
// Value описывает значение динамического типа для metadata
type Value struct {
	state protoimpl.MessageState `protogen:"open.v1"`
...
const file_inventory_v1_inventory_proto_rawDesc = "" +
	...
	"\tListParts\x12\x1e.inventory.v1.ListPartsRequest\x1a\x1f.inventory.v1.ListPartsResponseBRZPgithub.com/paincake00/microservices-go/shared/pkg/proto/inventory/v1;inventoryv1b\x06proto3"
